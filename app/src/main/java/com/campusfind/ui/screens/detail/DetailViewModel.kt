...
class DetailViewModel @Inject constructor(
    ...
    fun loadItem(itemId: String) {
        ...
        itemJob = viewModelScope.launch {
            lostItemRepository.observeItemById(itemId)
                ...
                // retryWhen keeps the Flow alive if a transient error occurs.
                // Without this, a single exception kills the coroutine and the
                // UI stops updating forever — requiring an app restart.
                ...
                .collect { item ->
                    ...
                    // Start tips and claims observers only once
    ...
    fun submitReply(parentTipId: String, message: String) {
        ...
        if (userId != null) {
            ...
            if (userReplyCount >= MAX_REPLIES_PER_USER_PER_ITEM) {
                _uiState.update { it.copy(showLimitDialog = "You've reached the maximum of $MAX_REPLIES_PER_USER_PER_ITEM replies on this item.\n\nThis limit helps us keep the app free for everyone! 🎉") }
                return
            }
        }
        ...
        if (trimmedMessage.length < MIN_TIP_LENGTH) {
            _uiState.update { it.copy(showLimitDialog = "Your reply is too short!\n\nPlease write at least $MIN_TIP_LENGTH characters. 💬") }
            return
        }
        if (trimmedMessage.length > MAX_TIP_LENGTH) {
            _uiState.update { it.copy(showLimitDialog = "Your reply is too long!\n\nPlease keep it under $MAX_TIP_LENGTH characters. ✂️") }
            return
        }
    ...
    fun approveClaim(claimId: String) {
        ...
        viewModelScope.launch {
            ...
            if (result.isFailure) {
                _uiState.update { it.copy(error = result.exceptionOrNull()?.message ?: "Failed to approve claim") }
            }
        }
    }
    ...
    fun replyToClaim(claimId: String, message: String) {
        ...
        if (userAlreadyReplied) {
            _uiState.update { it.copy(showLimitDialog = "You can only send 1 message per claim.\n\nUse Messenger for further coordination! 💬") }
            return
        }
        if (replies.size >= MAX_CLAIM_REPLIES_TOTAL) {
            _uiState.update { it.copy(showLimitDialog = "2-message limit reached.\n\nUse Messenger to continue the conversation! 💬") }
            return
        }
